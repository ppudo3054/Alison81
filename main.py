...
SUPPORT_USERNAME = "@ALISON"
SUPPORT_URL = "https://t.me/ALISON"
...
def generate_vless_link(
    uuid: str, host: str, remark: str = "ALISON",
    ...
    label = quote(str(remark or "ALISON"), safe="")
...
def get_link_info(
    ...
    return {
        ...
        "price_stars": int(link.get("price_stars") or 0),
...
async def make_link(
    ...
    record = {
        ...
        "client_limit": 0,
        "price_stars": max(0, int(price_stars or 0)),
        "usage_history": [],
    }
...
def _store_plan_cards(plans):
    ...
    for plan in plans:
        ...
        <article class="plan-card {'featured' if plan.get('featured') else ''}">
          ...
          <a class="buy-btn" href="https://t.me/{escape_html(os.environ.get('TELEGRAM_BOT_USERNAME','ALISONBot'))}?start=buy_{escape_html(plan.get('id',''))}">خرید از ربات فروش</a>
        </article>
        """)
    return "\n".join(cards)
...
<title>ALISON — فروش اشتراک</title>
<style>
...
</style></head><body><main class="wrap"><section class="hero"><div class="logo">V</div><h1>ALISON Store</h1><p class="sub">خرید سریع، تحویل خودکار و سابسکریپشن اختصاصی. پرداخت از طریق ربات فروش انجام می‌شود و بعد از پرداخت، لینک شما به‌صورت خودکار ساخته خواهد شد.</p></section><section class="grid">""" + _store_plan_cards(plans) + """</section><div class="note">پرداخت و تحویل توسط ربات رسمی ALISON انجام می‌شود. برای فعال‌سازی ربات، TELEGRAM_BOT_TOKEN و درگاه/Stars را تنظیم کنید.</div></main></body></html>"""

@app.get("/plans", response_class=HTMLResponse)
async def public_plans():
    if not SALES_ENABLED:
        return HTMLResponse("""<!doctype html><html lang="fa" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ALISON · Future Release</title><style>body{margin:0;min-height:100vh;display:grid;place-items:center;background:#070b13;color:#f5f7fb;font-family:Tahoma,Arial,sans-serif}.box{width:min(560px,calc(100% - 36px));padding:42px 28px;text-align:center;border:1px solid rgba(255,255,255,.1);border-radius:28px;background:linear-gradient(145deg,#0d1320,#111a2a);box-shadow:0 30px 90px rgba(0,0,0,.35)}.ico{width:72px;height:72px;margin:0 auto 18px;display:grid;place-items:center;border-radius:22px;background:rgba(124,92,255,.15);font-size:30px}.muted{color:#9aa7bc;line-height:2;font-size:13px}.tag{display:inline-block;margin-top:18px;padding:8px 13px;border-radius:999px;background:rgba(53,214,255,.08);border:1px solid rgba(53,214,255,.2);color:#64dcff;font-size:11px}</style></head><body><main class="box"><div class="ico">🔒</div><h1>فروش اشتراک موقتاً غیرفعال است</h1><p class="muted">ماژول فروش و پلن‌ها در نسخه فعلی ALISON فعال نیست. این قابلیت پس از تکمیل و تست نهایی در نسخه‌های بعدی منتشر خواهد شد.</p><span class="tag">ALISON · Future Release</span></main></body></html>""")
    import sales
...
async def api_system_diagnostics(request: Request, token=Depends(require_auth)):
    ...
    try:
        ...
        return {
            ...
            "service": {"status": "healthy", "version": "ALISON Pro"},
            "resources": {
...
async def create_link_api(
    ...
    price_stars = safe_int(body.get("price_stars", 0), minimum=0, maximum=100000000)
...
async def create_auto_link(
    ...
    uid, link = await make_link(
        ...
        note=f"Auto generated by ALISON | profile={profile}",
...
async def update_link(
    ...
    async with LINKS_LOCK:
        ...
        if "price_stars" in body:
            link["price_stars"] = max(0, min(100000000, safe_int(body.get("price_stars", 0), minimum=0)))
...
async def subscription_portal(uuid: str, request: Request):
    ...
    label = str(link.get("label") or "ALISON Subscription")
...
<meta name="theme-color" content="#070a12"><title>__LABEL__ · ALISON</title>
...
<header class="top">
  <div class="brand"><div class="logo">V</div><div><b>ALISON</b><small>SUBSCRIPTION CENTER</small></div></div>
...
<div class="footer">ALISON · وضعیت و مصرف به‌صورت زنده از سرویس خوانده می‌شود</div>
</main>
...
async def info_page(uid: str, request: Request):
    ...
    label = str(snapshot.get("label") or "ALISON")
...
<meta name="theme-color" content="#070a12"><meta name="color-scheme" content="dark"><title>__LABEL__ · ALISON</title>
...
<main class="wrap">
<div class="top"><div class="brand"><div class="mark">✦</div><div><b>ALISON</b><small>SECURE CLIENT PORTAL</small></div></div><div class="top-actions"><button class="btn" onclick="toggleTheme()">◐ پوسته</button><button class="btn" onclick="openQr()">▦ QR</button><span class="btn good">● __STATUS__</span></div></div>
...
<div class="footer">ALISON Secure Client Portal · اطلاعات اتصال فقط برای صاحب این لینک</div>
...
async def sub_group_subscription(
    ...
    group_title = (
        ...
        f"{sub['name']} | کانال تلگرام: ALISON"
    )
    headers = subscription_metadata_headers(
        total_used,
        group_limit,
...
<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#080b12"><title>ALISON · Subscription</title>
<style>
...
</style></head><body><main class="wrap"><div class="top"><div class="brand"><div class="mark">✦</div><div>ALISON<small>GROUP SUBSCRIPTION</small></div></div><div class="badge">● آماده استفاده</div></div><div id="app"></div><div class="footer">ALISON · Secure subscription delivery</div></main><div class="toast" id="toast">کپی شد</div>
<script>
...
async def api_approve_admin_request(req_id: str, request: Request, token=Depends(require_owner)):
    ...
    delivery_message = (
        ...
        f"حساب ادمین شما در ALISON فعال شد.\n\n"
